...
void CvCanny::onNewImage()
{
	...
	try {
		...
		cv::Mat thresh_im;
		if(autoThresh) {
			high_thresh = cv::threshold(in, thresh_im, 0, 255, cv::THRESH_BINARY + cv::THRESH_OTSU);
			higherThreshold = high_thresh;
